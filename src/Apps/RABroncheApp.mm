#include "RABroncheApp.h"
#include "dataLoader.h"
#import "AVFoundationVideoPlayer.h"

//--------------------------------------------------------------
RABroncheApp :: RABroncheApp () {
    cout << "creating RABroncheApp" << endl;
}

//--------------------------------------------------------------
RABroncheApp :: ~RABroncheApp () {
    cout << "destroying RABroncheApp" << endl;
}


//--------------------------------------------------------------
void RABroncheApp::setup(){
	ofBackground(0);
    ...
    video.loadMovie("VideosHD/Inhalation.mp4");
    video.play();
...
//    teapotImage.mirror(true, false);  //-- flip texture vertically since the texture coords are set that way on the teapot.
    ...
    */
    ...
    cadreB.loadImage("Interface/cadreB-ModeAction.jpg");
    
    animState = 0;
    animEnds[0] = 1000.0 / 1500;
    animEnds[1] = 1499.0 / 1500;
    

    model.loadModel(singletonDataLoader::Instance()->sceneBronche);
    model.optimizeScene();
...
//--------------------------------------------------------------
void RABroncheApp::draw(){
    ofSetColor(ofColor::white);

    if(video.isLoaded()) {
        video.getTexture()->draw(0,201, 2048, 1152);
    }
    else
    {
 ...
 if (animState == 1)
    {
        ...
        buttonSlected.loadImage("Interface/Boutons-JPG/Bouton-ModeAction01-x1435-y1389.jpg");
        buttonSlected.draw(1435,1389, 235, 89);
    }
...
//--------------------------------------------------------------
void RABroncheApp::touchDown(ofTouchEventArgs & touch){
//    touchPoint.set(touch.x, touch.y);

    int newState = 0;
    // DIFFUSION
    if (ofxQCAR::getInstance()->hasFoundMarker() && ofRectangle(1435,1389,235,89).inside(touch.x, touch.y))
         newState = 1;
    // ACCUEIL
...
//--------------------------------------------------------------
void RABroncheApp::touchMoved(ofTouchEventArgs & touch){
    touchPoint.set(touch.x, touch.y);
}

//--------------------------------------------------------------
void RABroncheApp::touchUp(ofTouchEventArgs & touch){
    touchPoint.set(-1, -1);
    
}

//--------------------------------------------------------------
void RABroncheApp::touchDoubleTap(ofTouchEventArgs & touch){
}

//--------------------------------------------------------------
void RABroncheApp::touchCancelled(ofTouchEventArgs & touch){
    
}

//--------------------------------------------------------------
void RABroncheApp::lostFocus(){
    
}

//--------------------------------------------------------------
void RABroncheApp::gotFocus(){
    
}

//--------------------------------------------------------------
void RABroncheApp::gotMemoryWarning(){
    
}

//--------------------------------------------------------------
void RABroncheApp::deviceOrientationChanged(int newOrientation){
